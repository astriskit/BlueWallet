...
import { ElectrumHistory } from '@/src/blue_modules/blue-electrum/types';
...
export class AbstractHDElectrumWallet extends AbstractHDWallet {
  ...
  _getNodeAddressByIndex(node: number, index: number): string {
    return this._getTransactionTrait._getNodeAddressByIndex.apply(this, [node, index]);
  }
  ...
  _getExternalAddressByIndex(index: number): string {
    return this._getTransactionTrait._getExternalAddressByIndex.apply(this, [index]);
  }

  _getInternalAddressByIndex(index: number) {
    return this._getTransactionTrait._getInternalAddressByIndex.apply(this, [index]);
  }
  ...
  async _binarySearchIterationForInternalAddress(index: number) {
    ...
    for (let c = 0; c < Math.round(index / this.gap_limit); c++) {
      const histories = await multiGetHistoryByAddress(gerenateChunkAddresses(c));
      // @ts-ignore
  ...
  async _binarySearchIterationForBIP47Address(paymentCode: string, index: number) {
    ...
    let lastHistoriesWithUsedAddresses = null;
    for (let c = 0; c < Math.round(index / this.gap_limit); c++) {
      const histories = await multiGetHistoryByAddress(generateChunkAddresses(c));
      // @ts-ignore
      if (this.constructor._getTransactionsFromHistories(histories).length > 0) {
        // in this particular chunk we have used addresses
        lastChunkWithUsedAddressesNum = c;
  ...
  async _fetchBalance() {
    ...
    const txs = await multiGetHistoryByAddress(lagAddressesToFetch); // <------ electrum call
    ...
    const balances = await multiGetBalanceByAddress(addresses2fetch);
  ...
  async fetchUtxo(): Promise<void> {
    ...
    const fetchedUtxo = await multiGetUtxoByAddress(addressess);
    this._utxo = [];
  ...
  _nodeToBech32SegwitAddress(hdNode: BIP32Interface): string {
    return this._getTransactionTrait._nodeToBech32SegwitAddress.apply(this, [hdNode]);
  }
  ...
  async wasEverUsed(): Promise<boolean> {
    const txs1 = await getTransactionsByAddress(this._getExternalAddressByIndex(0));
    if (txs1.length > 0) {
      return true;
    }
    if (!this.allowBIP47()) {
    ...
    const txs2 = await getTransactionsByAddress(address);
  ...
  getBIP47FromSeed(): BIP47Interface {
    return this._getTransactionTrait.getBIP47FromSeed.apply(this);
  }
  ...
  getBip47CounterpartyByTx(tx: Transaction): string | undefined {
    return this._getTransactionTrait.getBip47CounterpartyByTx.apply(this, [tx]);
  }
  ...
  async fetchBIP47SenderPaymentCodes(): Promise<void> {
    return this._getTransactionTrait.fetchBIP47SenderPaymentCodes.apply(this);
  }
  ...
  _hdNodeToAddress(hdNode: BIP32Interface): string {
    return this._getTransactionTrait._hdNodeToAddress.apply(this, [hdNode]);
  }
  ...
  _getBIP47AddressReceive(paymentCode: string, index: number): string {
    return this._getTransactionTrait._getBIP47AddressReceive.apply(this, [paymentCode, index]);
  }
