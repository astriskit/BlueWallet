...
import { calcEstimateFeeFromFeeHistorgam } from '../../src/blue_modules/blue-electrum/calcEstimateFeeFromFeeHistorgam';
import { testConnection } from '../../src/blue_modules/blue-electrum/testConnection';
import { estimateFee } from '../../src/blue_modules/blue-electrum/estimateFee';
...
import { multiGetTransactionByTxid, txhexToElectrumTransaction } from '../../src/blue_modules/blue-electrum/multiGetTransactionByTxid';
import { getTransactionsFullByAddress } from '../../src/blue_modules/blue-electrum/getTransactionsFullByAddress';
import { setBatchingDisabled } from '../../src/blue_modules/blue-electrum/setBatchingDisabled';
import { multiGetBalanceByAddress } from '../../src/blue_modules/blue-electrum/multiGetBalanceByAddress';
...
describe('BlueElectrum', () => {
  it('ElectrumClient can estimate fees from histogram', async () => {
    assert.strictEqual(
      calcEstimateFeeFromFeeHistorgam(1, [
        [96, 105086],
        [83, 124591],
        [64, 108207],
        [50, 131141],
    ...
    assert.strictEqual(
      calcEstimateFeeFromFeeHistorgam(18, [
        [96, 105086],
        [83, 124591],
        [64, 108207],
        [50, 131141],
    ...
    assert.strictEqual(
      calcEstimateFeeFromFeeHistorgam(144, [
        [96, 105086],
        [83, 124591],
        [64, 108207],
        [50, 131141],
  ...
  it('ElectrumClient can test connection', async () => {
    assert.ok(!(await testConnection('electrum1.bluewallet.io', 444, false)));
    assert.ok(!(await testConnection('electrum1.bluewallet.io', false, 444)));
    assert.ok(!(await testConnection('ya.ru', 444, false)));
    assert.ok(!(await testConnection('google.com', false, 80)));
    assert.ok(!(await testConnection('google.com', 80, false)));
    assert.ok(!(await testConnection('google.com', false, 443)));
    assert.ok(!(await testConnection('google.com', 443, false)));
    assert.ok(!(await testConnection('joyreactor.cc', false, 443)));
    assert.ok(!(await testConnection('joyreactor.cc', 443, false)));
    assert.ok(!(await testConnection('joyreactor.cc', 80, false)));
    assert.ok(!(await testConnection('joyreactor.cc', false, 80)));

    assert.ok(await testConnection('electrum1.bluewallet.io', '50001'));
    assert.ok(await testConnection('electrum1.bluewallet.io', false, 443));
  });

  it('ElectrumClient can estimate fees', async () => {
    assert.ok((await estimateFee(1)) > 1);
  ...
  it('BlueElectrum can do getTransactionsByAddress()', async function () {
    const txs = await getTransactionsByAddress('bc1qt4t9xl2gmjvxgmp5gev6m8e6s9c85979ta7jeh');
  ...
  it.skip('BlueElectrum can do getMempoolTransactionsByAddress()', async function () {
    const txs = await getMempoolTransactionsByAddress('bc1qp33en9mnw277c9vz5fz9vcu666cvervdnk02327wwph97hdjurqqxtl03c');
    assert.ok(txs.length > 0);
    assert.ok(txs[0].tx_hash);
    assert.ok(txs[0].fee);

    const rez = await multiGetTransactionByTxid([txs[0].tx_hash], 10, true);
    assert.ok(rez[txs[0].tx_hash]);
  });

  it('BlueElectrum can do getTransactionsFullByAddress()', async function () {
    const txs = await getTransactionsFullByAddress('bc1qt4t9xl2gmjvxgmp5gev6m8e6s9c85979ta7jeh');
    for (const tx of txs) {
  ...
  it('BlueElectrum can do txhexToElectrumTransaction()', () => {
    ...
    const decoded = txhexToElectrumTransaction(tx);
  ...
  it.each([false, true])('BlueElectrum can do multiGetBalanceByAddress(), disableBatching=%p', async function (diableBatching) {
    if (diableBatching) setBatchingDisabled();
    const balances = await multiGetBalanceByAddress([
    ...
    if (diableBatching) setBatchingEnabled();
  });

  it('BlueElectrum can do multiGetUtxoByAddress()', async () => {
    const utxos = await multiGetUtxoByAddress(
      [
        'bc1qt4t9xl2gmjvxgmp5gev6m8e6s9c85979ta7jeh',
        'bc1qvd6w54sydc08z3802svkxr7297ez7cusd6266p',
        'bc1qwp58x4c9e5cplsnw5096qzdkae036ug7a34x3r',
  ...
  it.each([false, true])('ElectrumClient can do multiGetHistoryByAddress(), disableBatching=%p', async disableBatching => {
    ...
    const histories = await multiGetHistoryByAddress(
      [
        'bc1qt4t9xl2gmjvxgmp5gev6m8e6s9c85979ta7jeh',
        'bc1qvd6w54sydc08z3802svkxr7297ez7cusd6266p',
        'bc1qwp58x4c9e5cplsnw5096qzdkae036ug7a34x3r',
  ...
  it.each([false, true])('ElectrumClient can do multiGetTransactionByTxid(), disableBatching=%p', async disableBatching => {
    if (disableBatching) setBatchingDisabled();
    const txdatas = await multiGetTransactionByTxid(
      [
        'ad00a92409d8982a1d7f877056dbed0c4337d2ebab70b30463e2802279fb936d',
  ...
  it.skip('multiGetTransactionByTxid() can work with huge tx', async () => {
    ...
    const txdatas = await multiGetTransactionByTxid(['484a11c5e086a281413b9192b4f60c06abf745f08c2c28c4b4daefe6df3b9e5c'], true);
  ...
  it.each([false, true])('ElectrumClient can do multiGetHistoryByAddress() to obtain txhex, disableBatching=%p', async disableBatching => {
    if (disableBatching) setBatchingDisabled();
    const txdatas = await multiGetTransactionByTxid(['881c54edd95cbdd1583d6b9148eb35128a47b64a2e67a5368a649d6be960f08e'], false, 3);
